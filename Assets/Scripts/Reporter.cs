...
public class Reporter : MonoBehaviour
{
	...
	public void Initialize()
	{
		if (!created)
		{
			...
			graphContent = new GUIContent(string.Empty, images.showGraphImage, "Show Graph");
			...
			cameraContent = new GUIContent(string.Empty, images.cameraImage, "Select Photo");
			...
			if ((log.logType != _LogType.Log || showLog) && (log.logType != _LogType.Warning || showWarning) && (log.logType != 0 || showError) && (log.logType != _LogType.Assert || showError) && (log.logType != _LogType.Exception || showError))
			{
				...
				if (log == selectedLog)
				{
					...
					if (showScene)
					{
						...
						GUI.Label(sceneLabelRect, scenes[sample.loadedScene], gUIStyle);
					}
					if (showMemory)
					{
	...
	private void drawGraph()
	{
		...
		if (currentFrame != -1 && currentFrame < samples.Count)
		{
			...
			GUILayout.Space(size.x);
			GUILayout.Box(showSceneContent, nonStyle, GUILayout.Width(size.x), GUILayout.Height(size.y));
			GUILayout.Label(scenes[sample2.loadedScene], nonStyle);
			GUILayout.Space(size.x);
			GUILayout.Box(showMemoryContent, nonStyle, GUILayout.Width(size.x), GUILayout.Height(size.y));
	...
	private void drawStack()
	{
		if (selectedLog != null)
		{
			...
			GUILayout.Space(size.x);
			GUILayout.Box(showSceneContent, nonStyle, GUILayout.Width(size.x), GUILayout.Height(size.y));
			GUILayout.Label(scenes[sample.loadedScene], nonStyle);
			GUILayout.Space(size.x);
			GUILayout.Box(showMemoryContent, nonStyle, GUILayout.Width(size.x), GUILayout.Height(size.y));
	...
	private void Update()
	{
		...
		float num2 = Time.realtimeSinceStartup - lastUpdate2;
		if (num2 > 1f)
		{
	...
	private IEnumerator readInfo()
	{
		...
		if (Application.platform != RuntimePlatform.OSXPlayer && Application.platform != RuntimePlatform.WindowsPlayer && !url.Contains("://"))
		{
			url = "file://" + url;
		}
		WWW www = new WWW(url);
