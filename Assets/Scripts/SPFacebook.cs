...
public class SPFacebook : SA_Singleton<SPFacebook>
{
	...
	private void ParseFriendsFromJson(string data, Dictionary<string, FB_UserInfo> friends, bool invitable = false)
	{
		...
		try
		{
			...
			if (invitable)
			{
				IDictionary dictionary2 = dictionary["invitable_friends"] as IDictionary;
				obj = dictionary2;
			}
			else
			{
				obj = dictionary["friends"] as IDictionary;
			}
			IDictionary dictionary3 = (IDictionary)obj;
			IList list = dictionary3["data"] as IList;
