...
namespace StansAssets.Animation
{
	public class SA_iTween : MonoBehaviour
	{
		...
		public static void ColorFrom(GameObject target, Hashtable args)
		{
			...
			if ((bool)target.GetComponent<UnityEngine.UI.Image>())
			{
				color2 = (color = target.GetComponent<UnityEngine.UI.Image>().color);
			}
			else if ((bool)target.GetComponent<UnityEngine.UI.Text>())
			{
		...
		{
			if ((bool)GetComponent<UnityEngine.UI.Image>())
			{
				colors = new Color[1, 3];
				colors[0, 0] = (colors[0, 1] = GetComponent<UnityEngine.UI.Image>().color);
			}
			else if ((bool)GetComponent<UnityEngine.UI.Text>())
			{
				colors = new Color[1, 3];
		...
		private void ApplyColorToTargets()
		{
			...
			{
				if ((bool)GetComponent<UnityEngine.UI.Text>())
				{
					return;
				}
				if ((bool)GetComponent<Renderer>())
		...
		public static void CameraFadeSwap(Texture2D texture)
		{
			if ((bool)cameraFade)
			{
				...
				cameraFade.GetComponent<UnityEngine.UI.Image>().sprite = sprite;
			}
		}
		...
		private void ConflictCheck()
		{
			...
			while (true)
			{
				...
				if (sA_iTween.isRunning && sA_iTween.type == type)
				{
					...
					{
						if (!sA_iTween.tweenArguments.Contains(tweenArgument.Key))
						{
							sA_iTween.Dispose();
							return;
						}
						if (!sA_iTween.tweenArguments[tweenArgument.Key].Equals(tweenArguments[tweenArgument.Key]) && (string)tweenArgument.Key != "id")
						{
							sA_iTween.Dispose();
							return;
						}
