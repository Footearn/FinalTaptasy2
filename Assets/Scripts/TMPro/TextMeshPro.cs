	...
	public class TextMeshPro : TMP_Text, ILayoutElement
	{
		...
		private void UpdateMask()
		{
			if (m_isMaskingEnabled)
			{
				...
				m_fontMaterial.SetVector(ShaderUtilities.ID_ClipRect, vector2);
		...
		protected override void GenerateTextMesh()
		{
			...
			for (int i = 0; m_char_buffer[i] != 0; i++)
			{
				...
				Vector3 topLeft = new Vector3(0f + m_xAdvance + (m_cached_TextElement.xOffset - num5 - num6) * num2 * (1f - m_charWidthAdjDelta), (baseline + m_cached_TextElement.yOffset + num5) * num2 - m_lineOffset + m_baselineOffset, 0f);
			...
			case TextAlignmentOptions.TopJustified:
				a = ((m_overflowMode == TextOverflowModes.Page) ? (textContainerLocalCorners[1] + new Vector3(0f + margin.x, 0f - m_textInfo.pageInfo[num10].ascender - margin.y, 0f)) : (textContainerLocalCorners[1] + new Vector3(0f + margin.x, 0f - m_maxAscender - margin.y, 0f)));
				break;
			case TextAlignmentOptions.Left:
			case TextAlignmentOptions.Center:
			case TextAlignmentOptions.Right:
			case TextAlignmentOptions.Justified:
				a = ((m_overflowMode == TextOverflowModes.Page) ? ((textContainerLocalCorners[0] + textContainerLocalCorners[1]) / 2f + new Vector3(0f + margin.x, 0f - (m_textInfo.pageInfo[num10].ascender + margin.y + m_textInfo.pageInfo[num10].descender - margin.w) / 2f, 0f)) : ((textContainerLocalCorners[0] + textContainerLocalCorners[1]) / 2f + new Vector3(0f + margin.x, 0f - (m_maxAscender + margin.y + num14 - margin.w) / 2f, 0f)));
				break;
			case TextAlignmentOptions.BottomLeft:
			case TextAlignmentOptions.Bottom:
			case TextAlignmentOptions.BottomRight:
			case TextAlignmentOptions.BottomJustified:
				a = ((m_overflowMode == TextOverflowModes.Page) ? (textContainerLocalCorners[0] + new Vector3(0f + margin.x, 0f - m_textInfo.pageInfo[num10].descender + margin.w, 0f)) : (textContainerLocalCorners[0] + new Vector3(0f + margin.x, 0f - num14 + margin.w, 0f)));
				break;
			...
			case TextAlignmentOptions.MidlineJustified:
				a = (textContainerLocalCorners[0] + textContainerLocalCorners[1]) / 2f + new Vector3(0f + margin.x, 0f - (m_meshExtents.max.y + margin.y + m_meshExtents.min.y - margin.w) / 2f, 0f);
				break;
			}
			...
			int num37 = 0;
			int num38 = 0;
			float num39 = (m_previousLossyScaleY = transform.lossyScale.y);
			...
			for (int j = 0; j < m_characterCount; j++)
			{
				...
				if (isVisible)
				{
					...
					case TMP_TextElementType.Character:
					{
						...
						float num53 = characterInfo[j].scale * num39 * (1f - m_charWidthAdjDelta);
