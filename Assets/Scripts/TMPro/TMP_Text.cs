	...
	public class TMP_Text : MaskableGraphic
	{
		...
		protected virtual Vector2 CalculatePreferredValues(float defaultFontSize, Vector2 marginSize)
		{
			...
			for (int i = 0; m_char_buffer[i] != 0; i++)
			{
				...
				bool isUsingAlternateTypeface = m_internalCharacterInfo[m_characterCount].isUsingAlternateTypeface;
				float num10 = 1f;
				if (m_textElementType == TMP_TextElementType.Character)
				{
				...
				if (m_textElementType == TMP_TextElementType.Sprite)
				{
					...
					m_currentFontAsset = m_fontAsset;
					float num11 = m_currentFontSize / m_fontAsset.fontInfo.PointSize * m_fontAsset.fontInfo.Scale * ((!m_isOrthographic) ? 0.1f : 1f);
					num2 = m_fontAsset.fontInfo.Ascender / tMP_Sprite.height * tMP_Sprite.scale * num11;
				...
				if (m_monoSpacing != 0f)
				{
					num13 = m_monoSpacing / 2f - (m_cached_TextElement.width / 2f + m_cached_TextElement.xOffset) * num2;
					m_xAdvance += num13;
				}
				...
				float num14 = m_currentFontAsset.fontInfo.Ascender * ((m_textElementType != 0) ? m_internalCharacterInfo[m_characterCount].scale : num2) + m_baselineOffset;
				m_internalCharacterInfo[m_characterCount].ascender = num14 - m_lineOffset;
				m_maxLineAscender = ((!(num14 > m_maxLineAscender)) ? m_maxLineAscender : num14);
				float num15 = m_currentFontAsset.fontInfo.Descender * ((m_textElementType != 0) ? m_internalCharacterInfo[m_characterCount].scale : num2) + m_baselineOffset;
				float num16 = (m_internalCharacterInfo[m_characterCount].descender = num15 - m_lineOffset);
				...
				{
					float num17 = (num14 - m_baselineOffset) / m_currentFontAsset.fontInfo.SubSize;
					num14 = m_maxLineAscender;
					m_maxLineAscender = ((!(num17 > m_maxLineAscender)) ? m_maxLineAscender : num17);
					float num18 = (num15 - m_baselineOffset) / m_currentFontAsset.fontInfo.SubSize;
					num15 = m_maxLineDescender;
					m_maxLineDescender = ((!(num18 < m_maxLineDescender)) ? m_maxLineDescender : num18);
				}
				if (m_lineNumber == 0)
				{
					m_maxAscender = ((!(m_maxAscender > num14)) ? num14 : m_maxAscender);
				}
				if (num3 == 9 || !char.IsWhiteSpace((char)num3) || m_textElementType == TMP_TextElementType.Sprite)
				{
					float num19 = ((m_width == -1f) ? (x + 0.0001f - m_marginLeft - m_marginRight) : Mathf.Min(x + 0.0001f - m_marginLeft - m_marginRight, m_width));
					if (m_xAdvance + m_cached_TextElement.xAdvance * ((num3 == 173) ? num12 : num2) > num19 && enableWordWrapping && m_characterCount != m_firstCharacterOfLine)
					{
						...
						{
							float num20 = m_maxLineAscender - m_startOfLineAscender;
							AdjustLineOffset(m_firstCharacterOfLine, m_characterCount, num20);
							m_lineOffset += num20;
							state2.lineOffset = m_lineOffset;
							state2.previousLineAscender = m_maxLineAscender;
						}
						float num21 = m_maxLineAscender - m_lineOffset;
						float num22 = m_maxLineDescender - m_lineOffset;
						m_maxDescender = ((!(m_maxDescender < num22)) ? num22 : m_maxDescender);
						m_firstCharacterOfLine = m_characterCount;
						num7 += m_xAdvance;
						num8 = ((!m_enableWordWrapping) ? Mathf.Max(num8, num21 - num22) : (m_maxAscender - m_maxDescender));
						SaveWordWrappingState(ref state, i, m_characterCount - 1);
						m_lineNumber++;
						if (m_lineHeight == 0f)
						{
							float num23 = m_internalCharacterInfo[m_characterCount].ascender - m_internalCharacterInfo[m_characterCount].baseLine;
							num6 = 0f - m_maxLineDescender + num23 + (num5 + m_lineSpacing + m_lineSpacingDelta) * num;
							m_lineOffset += num6;
				...
				{
					m_xAdvance += m_monoSpacing - num13 + (m_characterSpacing + m_currentFontAsset.normalSpacingOffset) * num2 + m_cSpacing;
				}
				else
				{
				...
				if (num3 == 10 || m_characterCount == totalCharacterCount - 1)
				{
					...
					float num26 = m_maxLineDescender - m_lineOffset;
					m_maxDescender = ((!(m_maxDescender < num26)) ? num26 : m_maxDescender);
					...
					if (num3 == 10)
					{
						...
						if (m_lineHeight == 0f)
						{
							num6 = 0f - m_maxLineDescender + num14 + (num5 + m_lineSpacing + m_paragraphSpacing + m_lineSpacingDelta) * num;
							m_lineOffset += num6;
						}
						else
						{
				...
				if (m_enableWordWrapping || m_overflowMode == TextOverflowModes.Truncate || m_overflowMode == TextOverflowModes.Ellipsis)
				{
					...
					else if (num3 > 11904 && num3 < 40959 && !m_isNonBreakingSpace)
					{
						if (!TMP_Settings.linebreakingRules.leadingCharacters.ContainsKey(num3) && m_characterCount < totalCharacterCount - 1 && !TMP_Settings.linebreakingRules.followingCharacters.ContainsKey(m_internalCharacterInfo[m_characterCount + 1].character))
						{
							SaveWordWrappingState(ref state2, i, m_characterCount);
							m_isCharacterWrappingEnabled = false;
							flag = false;
		...
		protected virtual void FillCharacterVertexBuffers(int i, int index_X4)
		{
			...
			m_textInfo.meshInfo[materialReferenceIndex].vertices[0 + index_X4] = characterInfo[i].vertex_BL.position;
			...
			m_textInfo.meshInfo[materialReferenceIndex].uvs0[0 + index_X4] = characterInfo[i].vertex_BL.uv;
			...
			m_textInfo.meshInfo[materialReferenceIndex].uvs2[0 + index_X4] = characterInfo[i].vertex_BL.uv2;
			...
			m_textInfo.meshInfo[materialReferenceIndex].colors32[0 + index_X4] = characterInfo[i].vertex_BL.color;
		...
		protected bool ValidateHtmlTag(int[] chars, int startIndex, out int endIndex)
		{
			...
			case 41311:
			{
				...
				else
				{
					...
					{
						material = Resources.Load<Material>("Fonts & Materials/" + new string(m_htmlTag, m_xmlAttribute[1].valueStartIndex, m_xmlAttribute[1].valueLength));
						if (material == null)
						{
							return false;
						}
			...
			case 2246877:
			{
				...
				else
				{
					if (spriteAsset == null)
					{
						spriteAsset = Resources.Load<TMP_SpriteAsset>("Sprites/" + new string(m_htmlTag, m_xmlAttribute[0].valueStartIndex, m_xmlAttribute[0].valueLength));
					}
					if (spriteAsset == null)
					{
						return false;
